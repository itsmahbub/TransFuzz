...
- **Precomputed Experiment Results**  
  ...
  *Paths:* `results.json`, `initial_coverage.json`
...
- `fault_counts_over_time.py` generates `Figure 2` by aggregating runtime statistics recorded during fuzzing (stored in the `outputs/` directory).  
  The generated figure is saved as `analysis/fault_counts_over_time.png`.

- `plot_images_best_worst_v2.py` generates `Figure 3` by analyzing fault-inducing inputs produced during fuzzing and ranking them using perceptual similarity metrics (LPIPS).  
  The script is provided to document the exact procedure used to generate the figure in the paper.  
  Reproducing `Figure 3` requires the corresponding generated adversarial inputs, which are not included in the artifact due to storage constraints.  
  The generated figure is saved as `analysis/naturalness-qualitative.png`.


## Representative Fault-Inducing Examples

The `outputs/resnet50/None/ImageNet/NLC/None/24/0/aes` directory contains a small subset of fault-inducing inputs generated by TransFuzz for qualitative inspection.
