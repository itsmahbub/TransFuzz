...
class SurpriseCoverage(Coverage):
    ...
    def build_SA(self, data_batch, label_batch):
        ...
        layer_output_dict = tool.get_layer_output(self.model, data_batch, unroll=self.unroll, inference_func=self.inference_func, track_grad=self.track_grad)
        for (layer_name, layer_output) in layer_output_dict.items():
...
class NLC(Coverage):
    def init_variable(self, hyper=None):
        ...
        self.current = 1
        for (layer_name, layer_size) in self.layer_size_dict.items():
            # print(layer_name, layer_size)
            
            # if np.prod(layer_size) > 10_000:
    ...
    def calculate(self, data):
        ...
        for (layer_name, layer_output) in layer_output_dict.items():
            ...
            # if  layer_output.numel() > 10_000:
            #     # print(f'Warning: layer {layer_name} has {layer_output.numel()} elements, which may cause high memory usage.')
            #     continue
            if layer_name not in self.estimator_dict:
                continue
            # if "conv1d" in layer_name.lower():
            #     print("PRoblematic Conv1d layer skipped")
