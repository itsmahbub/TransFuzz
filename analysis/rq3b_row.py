...
MODELS = [
    ("mitast", "speech_commands", None),
    ("wav2vec2kws", "speech_commands", None)
]
...
def generate_coverage_rows(json_path, batch_size, random_mutation=False):
    ...
    for model_name, dataset_name, model_path in MODELS:
        ...
        new_cov = []
        for seed in SEEDS:
            key = f"{model_name}-{model_path}-{dataset_name}-{TARGET_LABELS[model_name]}-{batch_size}-{seed}"
            if random_mutation:
                key = key + "-rand"
            
            entry = data.get(key, {})
            faults.append(entry.get("generated_count", 0))
            lpipss.append(entry.get("naturalness", {}).get("mean_PESQ", 0))
